...
document.addEventListener('alpine:init', () => {
    Alpine.data('salaVideollamada', (reservaId) => ({
        ...
        async iniciar() {
            try {
                // Buscamos el token CSRF para seguridad
                const csrfToken = document.querySelector('meta[name="csrf-token"]')?.getAttribute('content');
                
                // Pedimos el "Pase VIP" a nuestra API de Laravel
                ...
                // Preparamos la Sala Virtual
                this.room = new Room();

               // Enganchamos su video a nuestra pantalla
                ...
                // Nos conectamos a los servidores de LiveKit Cloud
                await this.room.connect(data.url, data.token);
                this.conectado = true;
                this.cargando = false;

                // Intentamos prender la cámara (si tiene)
                try {
                ...
                // Intentamos prender el micrófono (si tiene)
                try {
